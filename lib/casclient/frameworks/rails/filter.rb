        ...
        class << self
          def filter(controller)
            ...
            if st
              ...
              if st.is_valid?
                #if is_new_session
                  log.info("Ticket #{st.ticket.inspect} for service #{st.service.inspect} belonging to user #{st.response.user.inspect} is VALID.")
                  controller.session[client.username_session_key] = st.response.user.dup
                  controller.session[client.extra_attributes_session_key] = HashWithIndifferentAccess.new(st.response.extra_attributes) if st.response.extra_attributes
                  
                  if st.response.extra_attributes
                    log.debug("Extra user attributes provided along with ticket #{st.ticket.inspect}: #{st.response.extra_attributes.inspect}.")
                  end
                  
                  # RubyCAS-Client 1.x used :casfilteruser as it's username session key,
                  # so we need to set this here to ensure compatibility with configurations
                  # built around the old client.
                  controller.session[:casfilteruser] = st.response.user
